...
export class FloatingMiniPalette {
  ...
  private renderStorage(panel: HTMLElement): void {
    ...
    const tagEdit = bottom.createEl("button", { text: "Tag edit" }); tagEdit.addEventListener("click", () => this.editSelectedTags());
  ...
  private renderLeftPane(parent: HTMLElement): void {
    ...
    parent.createEl("label", { text: "Card size" }); const cardSize = parent.createEl("input", { attr: { type: "range", min: "160", max: "360", value: String(this.plugin.store.data.settings.cardSize) } }); cardSize.addEventListener("input", () => { this.plugin.store.data.settings.cardSize = Number(cardSize.value); this.plugin.store.changed(); });
  ...
  private itemMenu(item: PaletteItem): void { const menu = new Menu(); menu.addItem((entry) => entry.setTitle("Open original").setIcon("external-link").onClick(() => void this.plugin.openOriginal(item))); menu.addItem((entry) => entry.setTitle("Delete").setIcon("trash").onClick(() => this.plugin.store.removeItems([item.id]))); menu.showAtPosition({ x: window.innerWidth / 2, y: window.innerHeight / 2 }); }
  private editSelectedTags(): void { const items = this.selectedIds().map((id) => this.plugin.store.data.items[id]).filter((item): item is PaletteItem => Boolean(item)); if (items.length === 0) { new Notice("Select items first."); return; } new TextPromptModal(this.plugin.app, "Set tags", items[0].tags.join(", "), (value) => { const tags = value.split(",").map((tag) => tag.trim().replace(/^#/, "")).filter(Boolean); for (const item of items) this.plugin.store.updateItem(item.id, { displayTitle: item.displayTitle, tags, label: item.label, caption: item.caption }); }, "tag1, tag2").open(); }
