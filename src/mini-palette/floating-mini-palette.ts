...
export class FloatingMiniPalette {
  ...
  private renderLeftPane(parent: HTMLElement, close?: () => void): void {
    ...
    parent.createEl("label", { text: "Filter type" }); const types = parent.createDiv({ cls: "cp-filter-chips" }); for (const type of ["all", "card", "markdown", "image", "video", "link", "group"] as const) { const button = types.createEl("button", { text: type === "all" ? "All" : type === "markdown" ? "MD" : type === "video" ? "Video" : type === "link" ? "Link" : type, cls: this.typeFilter === type ? "is-active" : "" }); button.addEventListener("click", () => { this.typeFilter = type; for (const chip of Array.from(types.querySelectorAll("button"))) chip.toggleClass("is-active", chip === button); this.refreshStorageItems(); }); }
  ...
  private renderStorageMain(parent: HTMLElement): void {
    ...
    if (this.layoutMode !== "wide" || !state.leftPaneOpen) {
      ...
      for (const value of ["all", "card", "markdown", "image", "video", "link", "group"] as const) type.createEl("option", { value, text: value === "all" ? "All types" : value === "markdown" ? "MD" : value === "video" ? "Video" : value === "link" ? "Link" : value });
