...
export class FloatingMiniPalette {
  ...
  private itemMenu(item: PaletteItem): void { const menu = new Menu(); menu.addItem((entry) => entry.setTitle("Edit metadata").setIcon("tags").onClick(() => new TagLabelModal(this.plugin.app, this.plugin, [item.id]).open())); menu.addItem((entry) => entry.setTitle("Open original").setIcon("external-link").onClick(() => void this.plugin.openOriginal(item))); menu.addItem((entry) => entry.setTitle("Delete").setIcon("trash").onClick(() => this.plugin.store.removeItems([item.id]))); menu.showAtPosition({ x: window.innerWidth / 2, y: window.innerHeight / 2 }); }
