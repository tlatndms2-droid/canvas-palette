...
export class FloatingMiniPalette {
  ...
  private renderStorageMain(parent: HTMLElement): void {
    ...
    for (const item of this.storageItems()) {
      const face = this.plugin.store.data.uiState.miniItemFaces[item.id] ?? "front";
      const card = renderItem(grid, item, { selected: this.selectedIds().includes(item.id), currentFace: face, onToggleFace: (next) => this.plugin.store.setPaletteFace("mini", item.id, next), draggable: true, onSelect: (event) => this.selectStorage(item.id, event.ctrlKey || event.metaKey), onOpen: () => face === "back" ? void this.plugin.editorManager.openBack(item.id) : void this.plugin.openItemEditor(item.id), onLocate: () => void this.plugin.locateItemOnCanvas(item), onContextMenu: (event) => { event.preventDefault(); this.itemMenu(item); } });
