...
export const DEFAULT_DATA: PaletteData = {
  ...
  uiState: { activeWorkspaceId: null, lastCanvasPath: null, selectedItemId: null, sideSelectedItemIds: [], sideItemFaces: {}, miniItemFaces: {}, quickEditor: { x: null, y: null, width: null, height: null }, workspaceExplorer: { viewMode: "details", sort: "modified-desc" }, miniPalette: {
    tab: "collect", storageWorkspaceFilter: null, isOpen: false, position: { x: 24, y: 62 }, size: { width: 1120, height: 720 },
...
export function migrateData(raw: Partial<PaletteData> | null | undefined): PaletteData {
  ...
  const hasStorageWorkspaceFilter = Object.prototype.hasOwnProperty.call(legacyUi.miniPalette ?? {}, "storageWorkspaceFilter");
  const requestedStorageWorkspaceFilter = hasStorageWorkspaceFilter ? legacyUi.miniPalette?.storageWorkspaceFilter ?? null : legacyUi.activeWorkspaceId ?? null;
  const storageWorkspaceFilter = requestedStorageWorkspaceFilter && workspaces[requestedStorageWorkspaceFilter] ? requestedStorageWorkspaceFilter : null;
  return {
