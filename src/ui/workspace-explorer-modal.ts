...
export class WorkspaceExplorerModal {
  ...
  private canvasArea(parent: HTMLElement): void {
    ...
    for (const path of [...paths].sort((a, b) => this.canvasName(a).localeCompare(this.canvasName(b)))) {
      ...
      const heading = folder.createDiv({ cls: "cp-workspace-canvas-folder__heading" }); const toggleButton = heading.createEl("button", { cls: "cp-workspace-canvas-folder__toggle", attr: { type: "button", "aria-label": expanded ? "Workspace 접기" : "Workspace 펼치기" } }); setIcon(toggleButton, expanded ? "chevron-down" : "chevron-right"); const icon = heading.createSpan(); setIcon(icon, "folder"); heading.createSpan({ text: this.canvasName(path) }); heading.createEl("small", { text: ` · ${workspaces.length}` }); const open = heading.createEl("button", { text: "Canvas 열기 ↗", cls: "cp-workspace-canvas-folder__open", attr: { type: "button", "aria-label": `${this.canvasName(path)} Canvas 열기` } });
      const toggle = () => { const paths = new Set(this.geo().expandedCanvasPaths); paths.has(path) ? paths.delete(path) : paths.add(path); this.geo().expandedCanvasPaths = [...paths]; this.plugin.store.changed(); }; toggleButton.addEventListener("click", toggle); this.dropTarget(heading, path); open.addEventListener("click", (event) => { event.stopPropagation(); void this.plugin.canvas.revealNode(path, ""); });
