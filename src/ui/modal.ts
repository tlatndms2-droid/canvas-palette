...
export class ConfirmDeleteCollectionModal extends Modal {
  ...
  onOpen(): void {
    ...
    const actions = this.contentEl.createDiv({ cls: "cp-modal-actions" });
    actions.createEl("button", { text: "Cancel" }).addEventListener("click", () => this.close());
    actions.createEl("button", { text: `Collection만 삭제 (${this.destination}로 이동)` }).addEventListener("click", () => { this.onRemoveCollectionOnly(); this.close(); });
    actions.createEl("button", { text: "Collection과 내부 항목 모두 삭제", cls: "mod-warning" }).addEventListener("click", () => { this.onRemoveEverything(); this.close(); });
  }
  onClose(): void { this.contentEl.empty(); }
}
