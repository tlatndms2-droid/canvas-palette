...
export class CardMarkdownExportModal extends Modal {
  constructor(app: App, private readonly initialName: string, private readonly initialFolder: string, private readonly onCreate: (fileName: string, folder: string) => Promise<boolean>) { super(app); }

  onOpen(): void {
    this.contentEl.addClass("canvas-palette", "cp-card-to-markdown-modal");
    this.contentEl.createEl("h2", { text: "Create Markdown file" });
    this.contentEl.createEl("p", { text: "Create a separate Markdown file from this Card. The Palette Card and every linked Canvas Card will remain unchanged." });
    ...
    create.addEventListener("click", async () => {
      ...
      try { if (await this.onCreate(fileName.value, folder.value)) this.close(); }
