...
test("cards expose only the deleted Markdown source state and keep existing Canvas-unlinked UI", async () => {
  ...
  assert.match(render, /deleted: "file-x"/);
  assert.doesNotMatch(render, /file-x-2/);
  ...
  assert.match(styles, /\.cp-item__header \.cp-source-state\{[^}]*width:22px;height:22px/);
  assert.match(styles, /\.cp-source-state svg\{width:13px;height:13px\}/);
  assert.match(styles, /var\(--text-error,#e53935\)/);
