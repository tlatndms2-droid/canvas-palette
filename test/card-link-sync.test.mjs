...
test("Group snapshot migration preserves legacy Front Back metadata and new per-node metadata", async () => {
  ...
  await store.load();
  assert.equal(store.data.schemaVersion, 32);
...
test("Outliner moves multiple items and safely reparents collections", async () => {
  ...
  store.moveItems("workspace", ["card"], "parent");
  store.moveCollection("child", "parent", "card", false);
  assert.deepEqual(store.outlineEntryIds("workspace", "parent"), ["child", "card"]);
