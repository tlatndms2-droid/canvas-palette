...
test("Image Front Back remains opt-in instead of being enabled by item type", async () => {
  ...
  assert.equal(data.schemaVersion, 30);
...
test("schema 22 does not turn the former automatic Mini browser into explicit relay links", async () => {
  ...
  const data = migrateData({
    schemaVersion: 20,
    items: {
      stored: { id: "stored", type: "card", displayTitle: "Stored", tags: [], label: "", caption: "", createdAt: 1, modifiedAt: 1, origin: {}, canvasPlacements: [], content: "Stored", backContent: "", facesEnabled: false }
    },
    pendingItemIds: [],
