...
test("selected Canvas text uses one Workspace-picker menu item", async () => {
  ...
  assert.match(main, /new TextScrapWorkspaceModal\(this\.app, workspaces, currentGeneralWorkspaceId/);
  assert.doesNotMatch(main, /Save text directly to Side Palette — \$\{currentWorkspace\.name\}/);
});

test("text Workspace picker defaults only the active general Workspace and requires a choice otherwise", async () => {
  ...
  assert.match(modal, /workspace\.id === this\.currentGeneralWorkspaceId && workspace\.kind === "general"/);
