...
test("visible Outliner Collection and file rows export through a dedicated mind-map tree", () => {
  ...
  assert.match(main, /async exportOutlineSelectionAsMindMap\(targets: OutlineSelectionTarget\[\]\)/);
  assert.match(main, /private exportOutlineSelectionTree\(targets: OutlineSelectionTarget\[\]\)/);
  assert.match(main, /selectedCollections\.has\(containingCollection\.id\)/);
  assert.match(side, /setTitle\("Export selection as MindMap to Canvas"\)[\s\S]{0,180}exportOutlineSelectionAsMindMap/);
  assert.match(side, /this\.outlineRows\.slice\(Math\.min\(start, end\), Math\.max\(start, end\) \+ 1\)/);
});
...
test("tree bundles retain hierarchy edges and apply depth-aware headings", () => {
  ...
  assert.match(adapter, /private exportCardText\(item: PaletteItem, headingLevel\?: number\)/);
  assert.match(adapter, /private syncedCardText\(item: PaletteItem, existingText: string\)/);
  assert.match(adapter, /private plainTitle\(value: string\)/);
