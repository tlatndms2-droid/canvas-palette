...
- Every linked Canvas node shows a link badge at its upper-left. A Palette card with no remaining linked Canvas node shows an Unlinked icon in a dedicated header status slot. The Canvas badge stays opposite the upper-right Front/Back control, while the Palette status shares a flex action slot with Front/Back and remains clear of the reserved selection marker.
...
- Image and Markdown context menus rename the linked Vault source file, Palette title, and every linked Canvas file-node path together. Group renaming updates the Palette title, stored root Group label, and every linked Canvas Group node. `Convert to shared Markdown…` creates exactly one real `.md` file in the Vault, changes the Palette Card into a Markdown Item whose source path is that file, and changes every linked Canvas text Card into a file node whose path is the same file. On an open Canvas the text node is removed and recreated in place as a file node so the visible runtime type changes immediately; its ID, bounds, edges, metadata, and link records are preserved.
