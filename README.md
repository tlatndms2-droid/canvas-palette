...
- Type-specific Card, Markdown, Image, and Group rendering; every Palette header combines its existing icon with a compact color-coded type label (`CARD`, `MD`, `IMAGE`, or `GROUP`) while leaving Canvas nodes unchanged. Rendered Markdown, source image preview, and subgraph preview keep their content-first card layouts.
