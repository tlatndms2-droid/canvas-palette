...
- Markdown source identity now follows Vault file and folder renames automatically. Palette cards expose only three compact, icon-only states beside the existing Front/Back button: green linked, red source deleted, and gray not placed on Canvas. The icons use the same footprint as the refresh control so the body stays primary; hover text explains each state. Deleted Markdown retains its last path and cached body, can still be placed as a Canvas text fallback, and its red status menu offers `Restore Markdown` or `Delete from Palette`. Legacy Markdown records with no source path are repaired into ordinary Cards instead of exposing an invalid path state.
