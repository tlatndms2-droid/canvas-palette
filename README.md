...
- Image and Markdown context menus rename the linked Vault source file, Palette title, and every linked Canvas file-node path together. Group renaming updates the Palette title, stored root Group label, and every linked Canvas Group node. `Convert to Markdown…` creates one `.md` file from a Card's synchronized content, converts the Palette Card into a Markdown Item, and converts every linked Canvas text Card into a file node that points to that same Markdown file while preserving the Item identity, metadata, node IDs, positions, edges, and link records.
