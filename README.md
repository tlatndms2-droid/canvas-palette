...
- Front/Back is opt-in per selected Canvas text or file node, including an Image only when the user explicitly enables that exact material; unrelated cards, images, and files remain unchanged. An enabled linked material shows its flip control across Canvas, Side Palette, and Mini Palette. The native node toolbar then exposes `Remove Front / Back`, which returns linked placements to Front and removes their flip UI without deleting the saved Back text. Re-enabling restores that Back. Canvas Back editing happens in place through Obsidian's native Live Preview, with the editor text anchored to the same upper-left position used by the rendered Back.
